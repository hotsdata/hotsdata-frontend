...
class InfernalShrines extends React.Component {
  constructor(props) {
    super(props)
  ...
  createStatsRow(playerInfo, playerStats) {
    ...
    return (
      <tr key={playerStats.playerId} className={(isWinner ? 'victory-row' : 'defeat-row')}>
        ...
        <td>{playerStats.DamageDoneToShrineMinions}</td>
      </tr>
    )
  }
      ...
      <table className="table">
        <thead>
          <tr>
            <th>Player</th>
            <th>Hero</th>
            <th>Damage Done to Shrine Minions</th>
          </tr>
        </thead>
        <tbody>
          {statRows}
...
export default InfernalShrines;
